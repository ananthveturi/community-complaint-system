...
def ensure_schema():
    ...
    try:
        ...
        if 'integrity_status' not in columns:
            conn.execute("ALTER TABLE complaints ADD COLUMN integrity_status TEXT DEFAULT 'ACCEPTED'")
        if 'risk_score' not in columns:
            conn.execute('ALTER TABLE complaints ADD COLUMN risk_score REAL DEFAULT 0.0')
